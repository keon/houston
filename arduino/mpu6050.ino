...
void setup() {
    ...
    while(!ms.begin(MS5611_ULTRA_HIGH_RES))
    {
    ...
    // Serial.println(hmc.testConnection() ? "HMC5883L connection successful" : "HMC5883L connection failed");
    // wait for ready
...
void loop() {
    ...
    // otherwise, check for DMP data ready interrupt (this should happen frequently)
        ...
        double realTemperature = ms.readTemperature();
        double realPressure = ms.readPressure();

        // Calculate sealevel pressure
        double seaLevelPressure = ms.getSeaLevel(realPressure, myRealAltitude);
